...
def add_polca(map,pix,nside,radius,val=2):
    n3d=hp.pix2vec(nside, pix)
    pixpolca=hp.query_disc(nside,n3d,radius)
    map[pixpolca]=val
    return

#def ChangeColor(nvec,nu,vvec):
...
hp.mollview(map_1,cmap='bwr',min=0,max=2)
plt.show()

hp.mollview(hp.ud_grade(map_1,nside2),cmap='bwr',min=0,max=2)
plt.show()
...
hp.mollview(hp.ud_grade(map_2,nside2),cmap='bwr',min=0,max=2)
plt.show()

hp.mollview(hp.ud_grade(map_3,nside2),cmap='bwr',min=0,max=2)
plt.show()

hp.mollview(hp.ud_grade(map_4,nside2),cmap='bwr',min=0,max=2)
plt.show()

hp.mollview(hp.ud_grade(map_5,nside2),cmap='bwr',min=0,max=2)
plt.show()

hp.orthview(hp.ud_grade(map_2,nside2),cmap='bwr',min=0,max=2)
plt.show()

hp.orthview(hp.ud_grade(map_3,nside2),cmap='bwr',min=0,max=2)
plt.show()

hp.orthview(hp.ud_grade(map_4,nside2),cmap='bwr',min=0,max=2)
plt.show()
